...
For example:
```sh
$ sudo sh buildeb.sh buster ftp://ftp.se.debian.org/debian/ "$(pwd)/buildarea"
...
docker run --privileged -v "$(pwd)":/opt/buildarea konstruktoid/debianbuild buster ftp://ftp.se.debian.org/debian/
```
