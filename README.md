...
`buildeb.sh` uses `debootstrap` to build a minimal Debian or Ubuntu root
...
```sh
$ buildarea="$(mktemp -d)"
$ sudo sh buildeb.sh bookworm http://deb.debian.org/debian "${buildarea}"
$ docker build -t konstruktoid/debian -f "${buildarea}/Dockerfile.bookworm" "${buildarea}"
...
docker run --privileged -v "${buildarea}":/opt/buildarea \
  konstruktoid/debianbuild bookworm http://deb.debian.org/debian
docker build -t konstruktoid/debian \
  -f "${buildarea}/Dockerfile.bookworm" "${buildarea}"
```
...
Previously generated images live on their own branches: `buster`, `jessie`,
`stretch` and `wheezy`.
